          ...
          (function () {
            ...
            L.Routing.GraphHopper = L.Evented.extend({
                  ...
                  L.bind(function (err, resp) {
                    ...
                    if (!timedOut) {
                      ...
                      this.fire("response", {
                        status: fired.status,
                        limit: Number(
                          fired.getResponseHeader("X-RateLimit-Limit")
                        ),
                        remaining: Number(
                          fired.getResponseHeader("X-RateLimit-Remaining")
                        ),
                        reset: Number(
                          fired.getResponseHeader("X-RateLimit-Reset")
                        ),
                        credits: Number(
                          fired.getResponseHeader("X-RateLimit-Credits")
                        ),
                      });
                      if (!err) {
              ...
              _mapWaypointIndices: function (
                ...
                wpIndices.push(0);
                wps.push(
                  new L.Routing.Waypoint(coordinates[0], waypoints[0].name)
                );
